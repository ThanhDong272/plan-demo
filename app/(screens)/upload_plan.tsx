...
const UploadPlan: React.FC<Props> = () => {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const { createPlan, deletePlan, createPlanDocument } = usePlan({
    planId: queryClient.getQueryData(["planIdCreated"]),
  });
  ...
  useEffect(() => {
    queryClient.resetQueries({ queryKey: ["planIdCreated"], exact: true });
  }, []);
  ...
  const handleSubmit = () => {
    if (title !== "" && date !== null) {
      createPlan.mutate({ title, date: dayjs(date).format("YYYY-MM-DD") });
    }
  };
  ...
  return (
    <Container>
      <RowBetweenView>
        ...
        {queryClient.getQueryData(["planIdCreated"]) ? (
          <Menu
            opened={openMenu}
            ...
            <MenuOptions optionsContainerStyle={styles.menuOptions}>
              <MenuOption
                onSelect={() => {
                  ...
                  deletePlan.mutate({
                    id: queryClient.getQueryData(["planIdCreated"]) as string,
                  });
                }}
              >
